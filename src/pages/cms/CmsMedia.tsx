...
export default function CmsMedia() {
  ...
  const remove = async (path: string) => { if (!confirm("Hapus file?")) return; await deleteFile(bucket, path); toast.success("Dihapus"); refresh(); };
          ...
          <Card key={f.path} className="overflow-hidden rounded-xl border-0 shadow-soft">
            ...
            <CardContent className="space-y-2 p-3">
              ...
              <div className="flex gap-1">
                ...
                <Button size="icon" variant="ghost" className="text-destructive" onClick={() => remove(f.path)}><Trash2 className="h-4 w-4" /></Button>
              </div>
            </CardContent>
          </Card>
        ))}
