...
export default function CmsPages() {
  const { data, refetch } = useSupabaseTable<any>("cms_pages");
          ...
          <Card key={p.id} className="rounded-2xl border-0 shadow-soft">
            <CardContent className="p-5">
              ...
              <div className="mt-3 flex gap-1">
                ...
                <Button size="icon" variant="ghost" className="text-destructive" onClick={async () => { if (confirm("Hapus?")) { await dbDelete("cms_pages", p.id); refetch(); } }}><Trash2 className="h-4 w-4" /></Button>
              </div>
            </CardContent>
          </Card>
        ))}
